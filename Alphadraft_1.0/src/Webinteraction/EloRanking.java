...
public class EloRanking {
	...
	public static void main (String[] args) throws IOException, NumberFormatException {
		//Team[] firstSeason =  EloRanking.createEloRanking("http://lol.esportspedia.com/wiki/2015_EU_Challenger_Series/Summer_Season/Match_History",null);
		EloRanking.createEloRanking("http://lol.esportspedia.com/wiki/2015_EU_Challenger_Series/Summer_Season/Match_History",null);
	}
    ...
    teamarray = Methods.mergeTeamList(previousSeason, teamarray);
     ...
     for (int i = 0;i < Methods.getObjectcountT(teamarray);i++){
        ...
        // In Alphadraft Team Roccat is just Roccat -> removing the " " and the "Team"
        for (int k2 = 0;k2 < Methods.getObjectcountT(teamarray);k2++){
        	if (Methods.containsIgnoreCase(teamarray[k2].name, "Team Roccat")){
        	teamarray[k2].name ="Roccat";
        	}
      	} 
        
        for (int k2 = 0;k2 < Methods.getObjectcountT(teamarray);k2++){
        	if (Methods.containsIgnoreCase(teamarray[k2].name, "Team Vitality")){
        	teamarray[k2].name ="Vitality";
        	}
      	}
        
        for (int k2 = 0;k2 < Methods.getObjectcountT(teamarray);k2++){
        	if (teamarray[k2].name.contains("-")){
        	teamarray[k2].name = teamarray[k2].name.replace("-", " ");
        	}
      	}
        
        for (int k2 = 0;k2 < Methods.getObjectcountT(teamarray);k2++){
        	if (teamarray[k2].name.equalsIgnoreCase("Giants Gaming")){
        	teamarray[k2].name = "Giants";
        	}
      	}
